    ...
    <div className="runtime-ops">
      ...
      <div className="runtime-two-column rollout">
        ...
        <form className="runtime-panel runtime-flag-form" onSubmit={saveFlag}>
          ...
          <div className="runtime-form-grid thirds"><label>Scope<select value={form.scopeType} onChange={event => setForm(item => ({ ...item, scopeType: event.target.value }))}><option value="GLOBAL">Global</option><option value="BRANCH">Branch</option><option value="PROCESS">Process</option><option value="LOB">LOB</option><option value="USER">User</option></select></label><label>Scope value{form.scopeType === 'BRANCH' ? <BranchSelect value={form.scopeValue || ''} onChange={next => setForm(item => ({ ...item, scopeValue: next }))} placeholder="Select branch" /> : form.scopeType === 'PROCESS' ? <ProcessSelect value={form.scopeValue || ''} onChange={next => setForm(item => ({ ...item, scopeValue: next }))} placeholder="Select process" /> : form.scopeType === 'LOB' ? <LobSelect value={form.scopeValue || ''} onChange={next => setForm(item => ({ ...item, scopeValue: next }))} placeholder="Select LOB" /> : <input disabled={form.scopeType === 'GLOBAL'} value={form.scopeValue || ''} onChange={event => setForm(item => ({ ...item, scopeValue: event.target.value }))} />}</label><label>Rollout %<input type="number" min="0" max="100" step="0.01" value={form.rolloutPercentage} onChange={event => setForm(item => ({ ...item, rolloutPercentage: event.target.value }))} /></label></div>
