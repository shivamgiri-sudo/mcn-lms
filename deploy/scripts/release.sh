...
if BASE_URL="${BASE_URL:-http://127.0.0.1:${LMS_HTTP_PORT:-4000}}" bash "$ROOT_DIR/deploy/scripts/smoke-test.sh"; then
...
if [[ -n "$PREVIOUS_IMAGE" ]]; then
  ...
  PREVIOUS_IMAGE="$PREVIOUS_IMAGE" bash "$ROOT_DIR/deploy/scripts/rollback.sh"
fi
exit 1
